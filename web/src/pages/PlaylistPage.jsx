...
export function PlaylistPage({ refresh }) {
  ...
  return (
    <LibraryPageFrame refresh={refresh} title={playlist?.name || "Playlist"} subtitle={subtitle} heroTrack={tracks[0]}>
      <PageSection title="Songs" subtitle={playlist?.description}>
        <TrackList tracks={tracks.map((track) => ({ ...track, playlistId, playlistName: playlist?.name }))} />
      </PageSection>
    </LibraryPageFrame>
  );
}
