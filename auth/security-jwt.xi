...
mapper issueToken(username: String, role: String) -> String {
    let header = base64UrlText("{\"alg\":\"HS256\",\"typ\":\"JWT\"}")
    let payload = base64UrlText("{\"sub\":" + jsonString(username) + ",\"role\":" + jsonString(role) + "}")
    let signingInput = header + "." + payload
    return signingInput + "." + jwtSignature(signingInput)
}

mapper verifyToken(token: String) -> AuthContext {
    ...
    let role = cleanRole(json.getString(payload, "role"))
